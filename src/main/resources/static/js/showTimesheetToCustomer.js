...
function approveTimesheet(id, customerkey) {
    $.post("/uren/approveTimesheet/" + id + "/" + customerkey, function () {
        ...
        document.getElementById("tsbuttons").innerHTML = '<button id="akkoord" class="btn btn-success" disabled>Akkoord</button>&nbsp;' +
            '<button id="afgekeurd" class="btn btn-danger" disabled>Niet akkoord</button>';
        }
    );
